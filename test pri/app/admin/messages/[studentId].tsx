...
export default function AdminChatScreen() {
  ...
  async function load() {
    ...
    await supabase.from('messages').update({ is_read: true }).eq('student_id', studentId).eq('sender', 'student');
  }
  ...
  async function send() {
    if (!text.trim()) return;
    await supabase.from('messages').insert({ student_id: studentId, sender: 'admin', message: text.trim() });
    setText('');
    load();
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Stack.Screen options={{ title: 'Chat', headerShown: true, headerStyle: { backgroundColor: '#0A0E1A' }, headerTintColor: 'white' }} />
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={[styles.bubble, item.sender === 'admin' ? styles.myBubble : styles.theirBubble]}>
              <Text style={styles.bubbleText}>{item.message}</Text>
            </View>
          )}
        />
        <View style={styles.inputRow}>
          <TextInput style={styles.input} placeholder="Reply..." placeholderTextColor="#8A8FA3" value={text} onChangeText={setText} />
